...
_G.json10 = json        -- Global namespace
...
function json.decode(s, startPos)
	...
	assert(startPos <= string.len(s),
		'Unterminated JSON encoded object found at position ' .. startPos .. ' in string:\n' .. s)
	...
	while string.find(s, startChar, j + 1) ~= j + 1 do
		...
		if string.sub(s, i, j) == "\\u" then
			...
			elseif n < 0x800 then
				...
				x = string.char(0xE0 + (math.floor(n / 4096) % 0x10), 0x80 + (math.floor(n / 64) % 0x40),
					0x80 + (n % 0x40))
			end
			table.insert(t, x)
		else
