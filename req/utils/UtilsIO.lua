...
function io.remove_directory_and_files( path, verbose )
	...
	if not path then
		BLT:Log(LogLevel.ERROR, "[Error] paramater #1 to io.remove_directory_and_files, string expected, recieved " .. tostring(path))
		return false
	end

	if not file.DirectoryExists( path ) then
		BLT:Log(LogLevel.ERROR, "[Error] Directory does not exist: " .. path)
		return false
	end
	...
	if dirs then
		for k, v in pairs( dirs ) do
			...
			if not r then
				BLT:Log(LogLevel.ERROR, "[Error] Could not remove directory: " .. child_path)
				return false
			end
		end
	end
	...
	if files then
		for k, v in pairs( files ) do
			...
			if not r then
				BLT:Log(LogLevel.ERROR, "[Error] Could not remove file: " .. file_path .. ", " .. error_str)
				return false
			end
		end
	end
	...
	if not r then
		BLT:Log(LogLevel.ERROR, "[Error] Could not remove directory: " .. path)
		return false
	end
...
function io.save_as_json( data, path )
	...
	if data and count > 0 then
		...
		if file then
			...
			BLT:Log(LogLevel.ERROR, string.format("[Error] Could not save to file '%s', data may be lost!", path))
			return false
		end

	else
		BLT:Log(LogLevel.WARN, string.format("[Warning] Attempting to save empty data table to '%s', skipping...", path))
		return true
	end
